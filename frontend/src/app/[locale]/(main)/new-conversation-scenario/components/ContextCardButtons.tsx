...
export default function ContextCardButtons() {
  ...
  return (
    <div>
      ...
      <div className="mb-10 w-full flex justify-center flex-col sm:flex-row gap-4">
        {contextModes.map((contextMode) => {
          const isSelected = formState.contextMode === contextMode.value;
          return (
            <button
              type="button"
              className={`w-full md:w-1/3 box-border rounded-2xl flex flex-col items-start justify-center text-lg outline outline-2 outline-bw-20 cursor-pointer hover:bg-marigold-30/80 active:outline-none active:bg-marigold-30 disabled:pointer-events-none p-4 group ${isSelected ? 'outline-none bg-marigold-30' : ''}`}
